...
export default function POS() {
  ...
  const updateQty = (productId, qty) => {
    ...
    setCart((prev) =>
      prev.map((item) => (item.product === productId ? { ...item, quantity: clamped } : item))
    );
  };
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-8">Select products to create a sale</p>
      ...
      <div className="flex gap-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
              {products.map((p) => (
                ...
                >
                  ...
                  <p className="text-brand-600 font-bold mt-1">${p.price.toFixed(2)}</p>
        ...
        <div className="w-72 flex-shrink-0">
          <h3 className="text-sm font-semibold text-gray-600 mb-3">Current Cart</h3>
                ...
                {cart.map((item) => (
                  <div key={item.product} className="px-4 py-3">
                    <div className="flex justify-between items-start">
                      <p className="text-sm font-medium text-gray-800 flex-1 pr-2 leading-tight">{item.name}</p>
                      <button
